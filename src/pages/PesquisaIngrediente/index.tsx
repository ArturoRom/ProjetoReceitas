...
import PesquisaIngredientes from "./PesquisaIngrediente";
...
const PesquisaIngrediente = ()  => {
  const [dados, setDados] = useState<Receitas[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const handleInputChange = (event:any) => {
    setSearchTerm(event.target.value);
  };
  const fetchMealById = async (idMeal: string) => {
    try {
      const response = await axios.get(
        `https://www.themealdb.com/api/json/v1/1/lookup.php?i=${idMeal}`
      );
      const mealData: Receitas = response.data.meals[0];
      return mealData;
    } catch (error) {
      console.error('Erro coletando os dados:', error);
      return null;
    }
  };
  const fetchData = async () => {
    try {
      ...
      const mealsData: Receitas[] = response.data.meals;

      const additionalDataPromises = mealsData.map((meal) =>
        fetchMealById(meal.idMeal)
      );

      const additionalData = await Promise.all(additionalDataPromises);

      const filteredAdditionalData = additionalData.filter((data) => data !== null);

      if (filteredAdditionalData.every((data) => typeof data === 'object')) {
        setDados(filteredAdditionalData as Receitas[]);
      } else {
        console.error('Dados inválidos recebidos:', filteredAdditionalData);
      }
    } catch (error) {
      console.error('Erro coletando os dados:', error);
    }
  ...
  return (
    ...
    <section id="benefits" className="mx-auto min-h-full w-5/6 py-20">
      ...
      >
        ...
        >
          <HText>PESQUISAR RECEITA POR INGREDIENTE</HText>
          <input
          type="text"
...
export default PesquisaIngrediente;
